...
/*   Updated: 2024/04/29 16:57:12 by merdal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	game->wall_image = mlx_texture_to_image(game->mlxpointer,
			game->wall_texture);
	if (!game->wall_image)
		write(1, "Error converting wall tex to img", 32);
	game->floor_image = mlx_texture_to_image(game->mlxpointer,
			game->floor_texture);
	if (!game->floor_image)
		write(1, "Error converting floor tex to img", 33);
	game->collectable_image = mlx_texture_to_image(game->mlxpointer,
			game->collectable_texture);
	if (!game->collectable_image)
		write(1, "Error converting collectable tex to img", 38);
	game->player_image = mlx_texture_to_image(game->mlxpointer,
			game->player_texture);
	if (!game->player_image)
		write(1, "Error converting player tex to img", 34);
	game->exit_image = mlx_texture_to_image(game->mlxpointer,
			game->exit_texture);
	if (!game->exit_image)
